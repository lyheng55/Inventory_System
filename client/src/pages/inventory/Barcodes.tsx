...
const Barcodes: React.FC = () => {
  ...
  const [bulkGenerateOpen, setBulkGenerateOpen] = useState<boolean>(false);
  const [bulkGenerating, setBulkGenerating] = useState<boolean>(false);
  const [bulkResults, setBulkResults] = useState<BulkGenerateResponse | null>(null);
  ...
  const { data: productsData, isLoading, refetch } = useQuery<ProductsResponse>(
  ...
  const bulkGenerateMutation = useMutation(
    async (data: { type: string }) => {
      const response = await axios.post<BulkGenerateResponse>('/barcodes/bulk-generate', data);
      return response.data;
    },
    {
      onSuccess: (data) => {
  ...
  const handleProductFound = (product: Product): void => {
  ...
  const handleBarcodeGenerated = (barcodeData: BarcodeResponse): void => {
    queryClient.invalidateQueries('products-with-barcodes');
                  ...
                  <TableBody>
                    {productsData?.products?.map((product) => (
                      <TableRow key={product.id}>
                        ...
                        <TableCell>
                          {(product as any).category || (product.Category?.name || 'N/A')}
                        </TableCell>
                        <TableCell>
