...
const Stock: React.FC = () => {
  ...
  const [movementType, setMovementType] = useState<'adjustment' | 'transfer'>('adjustment');
  ...
  const { data: stockDataRaw, isLoading } = useQuery<StockResponse>(
  ...
  const stockData: StockResponse = stockDataRaw && stockDataRaw.stocks
  ...
  const { data: lowStockAlertsData } = useQuery<{ alerts: LowStockAlert[] }>(
    'lowStockAlerts',
    async () => {
  ...
  const lowStockAlerts = Array.isArray(lowStockAlertsData) ? lowStockAlertsData : (lowStockAlertsData?.alerts || []);

  const adjustStockMutation = useMutation(
    (stockData: any) => axios.post('/stock/adjust', stockData),
    {
      onSuccess: () => {
  ...
  const transferStockMutation = useMutation(
    (transferData: any) => axios.post('/stock/transfer', transferData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('stock');
        queryClient.invalidateQueries('lowStockAlerts');
  ...
  const handleOpenDialog = (type: 'adjustment' | 'transfer' = 'adjustment'): void => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>): void => {
    ...
    const stockData = {
      ...
      ...(movementType === 'transfer' && formData.toWarehouseId && { toWarehouseId: parseInt(formData.toWarehouseId) })
    };
            ...
            <TableBody>
              {stockData?.stocks?.map((stock) => (
                <TableRow key={stock.id}>
                  ...
                  <TableCell>
                    <Chip
                      label={stock.quantity <= (stock.Product?.reorderPoint || 0) ? t('stock.lowStock') : t('stock.inStock')}
                      color={stock.quantity <= (stock.Product?.reorderPoint || 0) ? 'warning' : 'success'}
                      size="small"
                    />
                  </TableCell>
                  <TableCell>
