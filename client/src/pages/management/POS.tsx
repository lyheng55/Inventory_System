...
const POS: React.FC = (): JSX.Element => {
  ...
  // Fetch warehouses
  const { data: warehousesData, isLoading: warehousesLoading, error: warehousesError, refetch: refetchWarehouses } = useQuery<Warehouse[]>(
    'warehouses',
    async () => {
  ...
  const { data: productsData, refetch: refetchProducts } = useQuery<ProductsResponse>(
  ...
  const voidSaleMutation = useMutation(
    async ({ saleId, voidReason }: { saleId: number; voidReason: string }) => {
  ...
  // Add product to cart
  const handleAddToCart = (product: AvailableProduct): void => {
  ...
  // Handle barcode scan
  const handleBarcodeScan = (product: Product): void => {
    if (product) {
  ...
  const handleUpdateQuantity = (productId: number, delta: number): void => {
    setCart(cart.map(item => {
  ...
  const handleUpdateDiscount = (productId: number, discount: string): void => {
    setCart(cart.map(item =>
  ...
  const handleCheckout = (): void => {
    ...
    const saleData = {
      ...
      items: cart.map(item => ({
        ...
        totalPrice: parseFloat(String((item.unitPrice * item.quantity) - (item.discount || 0)))
      })),
      subtotal: parseFloat(String(subtotal)) || 0,
      taxAmount: parseFloat(String(tax)) || 0,
      discountAmount: parseFloat(String(discountAmount)) || 0,
      totalAmount: parseFloat(String(total)) || 0,
      paymentMethod: paymentData.paymentMethod || 'cash',
      paymentAmount: parseFloat(String(paymentData.paymentAmount || total)) || 0,
  ...
  // Handle void sale
  const handleVoidSale = (saleId: number): void => {
  ...
  // Get receipt data
  const fetchReceipt = async (saleId: number): Promise<any> => {
    try {
  ...
  return (
    // @ts-ignore - Box accepts multiple children, TypeScript inference issue
    <Box>
      ...
      {warehousesError && (
        <Alert severity="error" sx={{ mb: 3 }}>
          {t('pos.errorLoadingWarehouses')}: {(warehousesError as Error).message || t('common.error')}
        </Alert>
      )}
                  ...
                  {salesLoading ? (
                    ...
                    sales.map((sale) => (
                      <TableRow key={sale.id}>
                        ...
                        <TableCell>${parseFloat(String(sale.totalAmount || 0)).toFixed(2)}</TableCell>
                        <TableCell>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              ...
              onChange={(e) => setPaymentData({ ...paymentData, paymentMethod: e.target.value as 'cash' | 'card' | 'other' })}
            ...
            <Box>
              ...
              {currentSale.SaleItems?.map((item, index) => (
                <Box key={index} sx={{ mb: 2 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {item.quantity} × ${parseFloat(String(item.unitPrice || 0)).toFixed(2)} = ${parseFloat(String(item.totalPrice || 0)).toFixed(2)}
                  </Typography>
                </Box>
              ))}
              <Divider sx={{ my: 2 }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                <Typography>{t('pos.subtotal')}:</Typography>
                <Typography>${parseFloat(String(currentSale.subtotal || 0)).toFixed(2)}</Typography>
              </Box>
              {parseFloat(String(currentSale.taxAmount || 0)) > 0 && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography>{t('pos.tax')}:</Typography>
                  <Typography>${parseFloat(String(currentSale.taxAmount || 0)).toFixed(2)}</Typography>
                </Box>
              )}
              {parseFloat(String(currentSale.discountAmount || 0)) > 0 && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography>{t('pos.discount')}:</Typography>
                  <Typography>-${parseFloat(String(currentSale.discountAmount || 0)).toFixed(2)}</Typography>
                </Box>
              )}
              <Divider sx={{ my: 1 }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography variant="h6">{t('pos.total')}:</Typography>
                <Typography variant="h6">${parseFloat(String(currentSale.totalAmount || 0)).toFixed(2)}</Typography>
              </Box>
              <Typography variant="body2" color="text.secondary">
                {t('pos.payment')}: {currentSale.paymentMethod} - ${parseFloat(String(currentSale.paymentAmount || 0)).toFixed(2)}
              </Typography>
              {parseFloat(String(currentSale.changeAmount || 0)) > 0 && (
                <Typography variant="body2" color="text.secondary">
                  {t('pos.change')}: ${parseFloat(String(currentSale.changeAmount || 0)).toFixed(2)}
                </Typography>
              )}
            </Box>
          )}
