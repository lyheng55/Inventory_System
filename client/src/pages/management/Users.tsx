...
const Users: React.FC = () => {
  ...
  const createUserMutation = useMutation(
    (userData: Partial<UserFormData>) => axios.post('/users', userData),
    {
      onSuccess: () => {
  ...
  const updateUserMutation = useMutation(
    ({ id, userData }: { id: number; userData: Partial<UserFormData> }) => axios.put(`/users/${id}`, userData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('users');
        queryClient.invalidateQueries('userStats');
  ...
  const changePasswordMutation = useMutation(
    ({ id, passwordData }: { id: number; passwordData: { currentPassword: string; newPassword: string } }) => axios.put(`/users/${id}/change-password`, passwordData),
    {
      onSuccess: () => {
  ...
  const reactivateUserMutation = useMutation(
    (id: number) => axios.put(`/users/${id}/reactivate`),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('users');
        queryClient.invalidateQueries('userStats');
  ...
  const getRoleColor = (role: string): 'error' | 'warning' | 'info' | 'secondary' | 'default' => {
    switch (role) {
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    {(() => {
                      ...
                      return (
                        <Chip
                          {...(roleIcon ? { icon: roleIcon as JSX.Element } : {})}
                  ...
                  <TableCell>{(user as any).createdAt ? formatDate((user as any).createdAt) : 'N/A'}</TableCell>
                  <TableCell>
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, isActive: e.target.checked })}
                      />
                    }
