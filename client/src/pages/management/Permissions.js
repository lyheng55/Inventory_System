...
const Permissions = () => {
  ...
  const [crudFlags, setCrudFlags] = useState({
    canCreate: false,
    canRead: true,
    canUpdate: false,
    canDelete: false
  ...
  const { data: userPermissionsData, refetch: refetchUserPerms, isLoading: userPermissionsLoading } = useQuery(
  ...
  const { data: rolePermissionsData, refetch: refetchRolePerms } = useQuery(
  ...
  const grantUserPermissionMutation = useMutation(
    ...
    },
    {
      onMutate: async ({ userId, permissionId, granted }) => {
        ...
        queryClient.setQueryData(['userPermissions', userId], (old) => {
          if (!old) return old;
          ...
          const updatedPermissions = permissionsArray.map(perm => {
  ...
  const userPermissions = Array.isArray(userPermissionsData?.permissions) ? userPermissionsData.permissions : 
                          Array.isArray(userPermissionsData) ? userPermissionsData : [];
  const rolePermissions = Array.isArray(rolePermissionsData?.permissions) ? rolePermissionsData.permissions : 
  ...
  const getUserPermissionStatus = (permission) => {
    ...
    const perm = userPerm.Permission || userPerm;
    ...
    return { 
      granted: granted, 
      source: perm.source || userPerm.source || 'user' 
    };
  };

  // Get permission status for role
  const getRolePermissionStatus = (permission) => {
    ...
    return {
      canCreate: perm.canCreate === true || rolePerm.canCreate === true,
      canRead: perm.canRead !== undefined ? perm.canRead : (rolePerm.canRead !== undefined ? rolePerm.canRead : true),
      canUpdate: perm.canUpdate === true || rolePerm.canUpdate === true,
      canDelete: perm.canDelete === true || rolePerm.canDelete === true
    };
  };

  const handleToggleUserPermission = (permission, event) => {
  ...
  const handleEditRolePermission = (permission) => {
    if (!permission || !permission.id) {
  ...
  const handleRevokeRolePermission = (permission) => {
    if (!selectedRole) return;
    setPermissionToRevoke(permission);
    setRevokeDialogOpen(true);
  };

  const confirmRevokeRolePermission = () => {
