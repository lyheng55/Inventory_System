...
const PurchaseOrders: React.FC = () => {
  ...
  const [newItem, setNewItem] = useState<PurchaseOrderItemFormData>({
    productId: '',
    quantity: '',
    unitPrice: '',
    expiryDate: '',
  ...
  const createOrderMutation = useMutation(
    (orderData: any) => axios.post('/purchase-orders', orderData),
    {
      onSuccess: () => {
  ...
  const updateOrderMutation = useMutation(
    ({ id, orderData }: { id: number; orderData: any }) => axios.put(`/purchase-orders/${id}`, orderData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('purchaseOrders');
        setOpenDialog(false);
  ...
  const approveOrderMutation = useMutation(
    (id: number) => axios.post(`/purchase-orders/${id}/approve`),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('purchaseOrders');
      }
  ...
  const cancelOrderMutation = useMutation(
    (id: number) => axios.delete(`/purchase-orders/${id}`),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('purchaseOrders');
      }
  ...
  const handleOpenDialog = (order: PurchaseOrder | null = null): void => {
    if (order) {
      setEditingOrder(order);
      setFormData({
        supplierId: String(order.supplierId),
        warehouseId: String(order.warehouseId),
        expectedDeliveryDate: order.expectedDeliveryDate ? String(order.expectedDeliveryDate).split('T')[0] : '',
        notes: order.notes || '',
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>): void => {
    ...
    const orderData = {
      ...
      items: formData.items.map(item => ({
        productId: typeof item.productId === 'number' ? item.productId : parseInt(String(item.productId)),
        quantity: typeof item.quantity === 'number' ? item.quantity : parseInt(String(item.quantity)),
            ...
            <TableBody>
              {ordersData?.orders?.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>${order.finalAmount ? parseFloat(String(order.finalAmount)).toFixed(2) : '0.00'}</TableCell>
                  <TableCell>
              ...
              <Box>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    <Typography>
                      {suppliers?.find(s => String(s.id) === formData.supplierId)?.name}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="subtitle2">{t('purchaseOrders.warehouse')}:</Typography>
                    <Typography>
                      {warehouses?.find(w => String(w.id) === formData.warehouseId)?.name}
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                        ...
                        return (
                          <ListItem key={index}>
                            <ListItemText
                              ...
                              secondary={`${quantity} × $${unitPrice.toFixed(2)} = $${(quantity * unitPrice).toFixed(2)}`}
                            />
                          </ListItem>
                        );
                      })}
              ...
              {tabValue === 0 && (
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <Typography variant="h6">${viewingOrder.finalAmount ? parseFloat(String(viewingOrder.finalAmount)).toFixed(2) : '0.00'}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="subtitle2">{t('purchaseOrders.supplier')}:</Typography>
                    ...
                    <TableBody>
                      {viewingOrder.PurchaseOrderItems?.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            <Chip
                              label={`${item.receivedQuantity || 0}/${item.quantity}`}
                              color={item.receivedQuantity === item.quantity ? 'success' : 'warning'}
                              size="small"
                            />
                          </TableCell>
                          <TableCell>${item.unitPrice ? parseFloat(String(item.unitPrice)).toFixed(2) : '0.00'}</TableCell>
                          <TableCell>${item.totalPrice ? parseFloat(String(item.totalPrice)).toFixed(2) : '0.00'}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
