...
const BackupRestore: React.FC = () => {
  ...
  const [keepCount, setKeepCount] = useState<number>(30);
  ...
  const restoreBackupMutation = useMutation(
    async ({ filename, options }: { filename: string; options: RestoreOptions }) => {
        ...
        <DialogContent>
          ...
          {createBackupMutation.isError && (
            <Alert severity="error" sx={{ mt: 2 }}>
              {(createBackupMutation.error as any)?.response?.data?.error || t('backupRestore.failedToCreateBackup')}
            </Alert>
          )}
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={restoreDialogOpen} onClose={() => setRestoreDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          {restoreBackupMutation.isError && (
            <Alert severity="error" sx={{ mt: 2 }}>
              {(restoreBackupMutation.error as any)?.response?.data?.error || t('backupRestore.failedToRestoreBackup')}
            </Alert>
          )}
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={cleanupDialogOpen} onClose={() => setCleanupDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          {cleanupMutation.isError && (
            <Alert severity="error" sx={{ mt: 2 }}>
              {(cleanupMutation.error as any)?.response?.data?.error || t('backupRestore.failedToCleanup')}
            </Alert>
          )}
          {cleanupMutation.isSuccess && (
            <Alert severity="success" sx={{ mt: 2 }}>
              {(cleanupMutation.data as any)?.message || t('backupRestore.cleanupCompleted')}
            </Alert>
          )}
        </DialogContent>
        <DialogActions>
