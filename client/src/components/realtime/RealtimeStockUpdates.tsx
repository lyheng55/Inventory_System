...
interface ExtendedStockUpdate extends StockUpdate {
  movementType?: 'in' | 'out' | 'adjustment';
  previousQuantity?: number;
  newQuantity?: number;
}

const RealtimeStockUpdates: React.FC = () => {
  const { stockUpdates, connected } = useRealtime();
  const [expanded, setExpanded] = useState<boolean>(false);

  const getMovementIcon = (movementType?: string): React.ReactNode => {
    switch (movementType) {
      case 'in':
  ...
  const getMovementColor = (movementType?: string): 'success' | 'error' | 'primary' => {
    switch (movementType) {
      case 'in':
        return 'success';
      case 'out':
  ...
  const getMovementLabel = (movementType?: string): string => {
    switch (movementType) {
      case 'in':
        return 'Stock In';
      case 'out':
  ...
  const extendedUpdates = stockUpdates as ExtendedStockUpdate[];
      ...
      <CardContent>
        ...
        <Collapse in={expanded}>
          {stockUpdates.length === 0 ? (
            ...
            <List sx={{ maxHeight: 300, overflow: 'auto' }}>
              {extendedUpdates.map((update, index) => (
                  ...
                  <ListItemText
                    ...
                    secondary={
                      <Box>
                        ...
                        {update.previousQuantity !== undefined && update.newQuantity !== undefined && (
                          <Typography variant="body2" color="text.secondary">
        ...
        {!expanded && stockUpdates.length > 0 && (
          <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
            ...
            <Box sx={{ ml: 1 }}>
              {extendedUpdates.slice(0, 3).map((update, index) => (
                <Chip
                  key={index}
                  label={getMovementLabel(update.movementType)}
                  size="small"
