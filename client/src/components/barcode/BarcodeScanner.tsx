...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ 
  ...
  const handleScan = async (barcodeValue: string): Promise<void> => {
    ...
    try {
      const response = await axios.post<ScanResponse>('/barcodes/scan', {
          ...
          <Card sx={{ bgcolor: 'success.light', color: 'success.contrastText' }}>
            <CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <Typography variant="body2" color="inherit">
                    <strong>Price:</strong> ${parseFloat((scannedProduct.unitPrice || 0).toString()).toFixed(2)}
                  </Typography>
                  <Typography variant="body2" color="inherit">
                    <strong>Unit:</strong> {scannedProduct.unit}
                  </Typography>
                  <Typography variant="body2" color="inherit">
                    <strong>Category:</strong> {(scannedProduct as any).category || 'N/A'}
                  </Typography>
                </Grid>
              </Grid>
