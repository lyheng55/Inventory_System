...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleUploadForFiles = async (filesToUpload: File[]): Promise<void> => {
    ...
    try {
      ...
      // Add additional data
      if (referenceId) {
        formData.append('productId', referenceId.toString());
      }
      if (description) {
  ...
  const handleRemoveExistingFile = async (filePath: string): Promise<void> => {
    try {
            ...
            {existingFiles.map((file, index) => (
              <ListItem key={index} divider>
                ...
                <ListItemSecondaryAction>
                  <IconButton
                    edge="end"
                    onClick={() => handleRemoveExistingFile(file.path || file.name || '')}
                    color="error"
                  >
                    <Delete />
                  </IconButton>
