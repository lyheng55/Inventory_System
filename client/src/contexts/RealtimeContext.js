...
export const RealtimeProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      newSocket.on('purchase-order-update', (update) => {
      ...
      newSocket.on('new-purchase-order', (notification) => {
      ...
      // Dashboard updates
      newSocket.on('dashboard-update', (update) => {
      ...
      // Product updates
      newSocket.on('product-update', (update) => {
      ...
      // Supplier updates
      newSocket.on('supplier-update', (update) => {
      ...
      // Warehouse updates
      newSocket.on('warehouse-update', (update) => {
  ...
  const joinWarehouse = (warehouseId) => {
    if (socket) {
      socket.emit('join-warehouse', warehouseId);
    }
  };

  const leaveWarehouse = (warehouseId) => {
    if (socket) {
